...
class RaidEventsTable(DynamoDBTable[RaidEvent]):
    ...
    def get_raid_event(self, guild_id: int, team_id: int, raid_name: str, raid_datetime: DateOptionalTime) -> Optional[RaidEvent]:
        return super(RaidEventsTable, self).get_item(guild_id=guild_id, team_id=team_id, name=raid_name, timestamp=raid_datetime)

    def get_raid_event_by_message_id(self, guild_id: int, team_id: int, message_id: DiscordMessageIdentifier, is_notification: bool) -> Optional[RaidEvent]:
        ...
        return self.to_unique_object(self.table.query(IndexName=RaidEventsTable.INDEX_NAME,
                                                      KeyConditionExpression=Key('guild_id#team_id').eq(f'{guild_id}#{team_id}'),
                                                      FilterExpression=Attr(attr_name).contains(message_id.to_str())))

    def list_raid_events(self, guild_id: int, team_id: Optional[int]) -> List[RaidEvent]:
        return self.table.query(IndexName=RaidEventsTable.INDEX_NAME, KeyConditionExpression=Key('guild_id#team_id').eq(f'{guild_id}#{team_id}'))
    ...
    def _to_object(self, item: Dict[str, Any]) -> RaidEvent:
        ...
        item['guild_id'], item['team_id'] = tuple(item['guild_id#team_id'].split('#'))
    ...
    def _to_item(self, raid_event: RaidEvent) -> Dict[str, Any]:
        ...
        item['guild_id#team_id'] = item['guild_id'] + '#' + item['team_id']
        del item['name']
        del item['timestamp']
        del item['guild_id']
        del item['team_id']
    ...
    def _to_key(self, guild_id: int, team_id: int, raid_name: str, raid_datetime: DateOptionalTime) -> Dict[str, Any]:
        return {
            'name#timestamp': f'{raid_name}#{raid_datetime.to_timestamp()}',
            'guild_id#team_id': f'{guild_id}#{team_id}'
        }
    ...
    def _table_kwargs(self):
        return {
            'KeySchema': [
                ...
                {
                    'AttributeName': 'guild_id#team_id',
                    'KeyType': 'RANGE'
                }
            ],
