...
class CommandRunner:
    ...
    async def _create_command(self, command_type: Type[BotCommand], argv: Optional[str] = None, message: Optional[discord.Message] = None,
        ...
        if raw_reaction:
            ...
            guild_member = get_member_by_id(discord_guild, user_id)
            channel = None
        elif message:
        ...
        return command_type(client=self.client, players_resource=self.players_resource, events_resource=self.events_resource,
                            guilds_resource=self.guilds_resource, message=None, raw_reaction=raw_reaction, argv=argv, member=guild_member, player=player,
                            discord_guild=discord_guild, guild=guild, raidgroup=raidgroup, channel=channel, logs_channel=logs_channel)
