...
def create_raidgroup(client: discord.Client, discord_guild: discord.Guild, member: GuildMember) -> RaidGroup:
    raidgroup_name = await interact(member, InteractionMessage(client, "Please fill in the name for your raiding group."))
    ...
    wl_group_id = await interact(member, InteractionMessage(client, msg))
