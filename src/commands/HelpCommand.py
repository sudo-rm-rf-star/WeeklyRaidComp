...
class HelpCommand(BotCommand):
    def __init__(self, name: str):
        subname = 'help'
        description = f"Toon alle commando's die beginnen met `{name}` en hoe ze op te roepen. Merk op, alle argumenten omringd door [ ] zijn optioneel en " \
                      f"kunnen weggelaten worden. "
        super(HelpCommand, self).__init__(name=name, subname=subname, description=description)
