...
class GuildsResource:
    ...
    @staticmethod
    def get_group(guild: Guild, player: Player) -> Optional[RaidTeam]:
        if guild is None:
            return None
        groups = [group for group in guild.raid_groups if group.id == player.selected_raidgroup_id or not player.selected_raidgroup_id]
