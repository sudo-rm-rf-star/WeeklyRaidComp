...
from dokbot.utils.CharacterSelectionInteraction import CharacterSelectionInteraction
...
class SignupCharacterCommand(CharacterCommand):
    ...
    async def execute(self, **kwargs) -> None:
        raid_event = RaidEventsResource().get_raid_by_message(self.message_ref)
        if not raid_event:
        ...
        if signup_choice == SignupStatus.SWITCH_CHAR:
            ...
            player, character = await CharacterSelectionInteraction.interact(member=self.member, client=self.client,
